...
class TestValidator(unittest.TestCase):
    ...
    def test_bound_port_checks_without_ignore(self, mock_out, mock_err):
        '''When ignore_listen_port is False (default case), an error should be raised
            if the ports are already bound.
        '''
        c = copy.deepcopy(config)
        ...
        VALIDATION_PARAMS['ignore_listen_port'] = False
    ...
    def test_bound_port_checks_with_ignore(self, mock_out, mock_err):
        '''When ignore_listen_port is True (when testing other validations except port),
            no error should be raised even if the ports are already bound.
        '''
        c = copy.deepcopy(config)
        c['restapi']['listen'] = "127.0.0.1:8000"
        c['postgresql']['listen'] = "127.0.0.1:9000"
        ...
        VALIDATION_PARAMS['ignore_listen_port'] = True
